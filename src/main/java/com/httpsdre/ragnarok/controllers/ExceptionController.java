...
public class ExceptionController {
  ...
  public ResponseEntity<ErrorDTO> unauthorizedException(UnauthorizedException e) {
    return ResponseEntity.status(401).body(new ErrorDTO(e.getMessage(), OffsetDateTime.now()));
  }

  @ExceptionHandler(NotFoundException.class)
  public ResponseEntity<ErrorDTO> notFoundException(NotFoundException e) {
    return ResponseEntity.status(404).body(new ErrorDTO(e.getMessage(), OffsetDateTime.now()));
  }

  @ExceptionHandler(UnprocessableEntityException.class)
  public ResponseEntity<ErrorDTO> unprocessableEntity(UnprocessableEntityException e) {
    return ResponseEntity.status(422).body(new ErrorDTO(e.getMessage(), OffsetDateTime.now()));
  }

  @ExceptionHandler(ValidationException.class)
  public ResponseEntity<ErrorDTO> validationException(ValidationException e) {
    return ResponseEntity.badRequest().body(new ErrorDTO(e.getMessage(), OffsetDateTime.now()));
  }
}
